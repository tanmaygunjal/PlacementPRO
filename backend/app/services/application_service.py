...
def apply_to_job(db: Session, student_id: int, application_schema: ApplicationCreate) -> Application:
    # 1. Check if job exists and is active
    job = db.query(Job).filter(Job.id == application_schema.job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="Job posting not found")
    if not job.is_active or job.deadline < datetime.utcnow():
        raise HTTPException(
    ...
    if not student_profile.resume_filename:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Resume not found. Please upload a resume before applying."
        )
        
    # 4. Check CGPA eligibility
    if student_profile.cgpa < job.eligibility_cgpa:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def update_application_status(db: Session, app_id: int, status_update: ApplicationStatus) -> Application:
